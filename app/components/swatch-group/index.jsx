...
import {Flex, Box, HStack, useStyleConfig, useMultiStyleConfig} from '@chakra-ui/react'
...
const SwatchGroup = (props) => {
    ...
    const stylePdpSwatchGroup = useMultiStyleConfig('PdpSwatchGroup')
    return (
        <Flex
            {...styles.swatchGroup}
            role="radiogroup"
        ...
        >
            <HStack {...styles.swatchLabel}>
                <Box {...stylePdpSwatchGroup.containerLabel}>{`${label}:`}</Box>
                <Box {...stylePdpSwatchGroup.containerDisplayName}>{displayName}</Box>
            </HStack>
